...
const NewDriverPage = () => {
    ...
    const { inviteDriver } = useDataContext()
    ...
    return (
        <Modal expandedOnLoad wrapContent onClose={onClose}>
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="flex justify-between">
                    <FormInput
                        className="w-36"
                        label="First Name"
                        inputProps={register(fields.FIRSTNAME)}
                        isError={errors[fields.FIRSTNAME]}
                    />
                    <FormInput
                        className="w-40"
                        label="Last Name"
                        inputProps={register(fields.LASTNAME)}
                        isError={errors[fields.LASTNAME]}
                    />
                </div>
                <PhoneInput
                    className="w-80"
                    ...
                    isError={errors[fields.EMAIL]}
                    control={control}
                />
                <div className="flex flex-row align-center">
                    <Link className="button-light mr-2" to="/drivers">
                        Cancel
                    </Link>
                    <ButtonWithSpinner className="button" loading={loading} disabled={!isValid}>
                        Submit
                    </ButtonWithSpinner>
                </div>
